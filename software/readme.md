...
```
git clone https://github.com/gboeing/ppd430.git
```

You now have a `ppd430` folder on your desktop containing the course repo.


### Step 2: Conda

Download and install [miniconda](https://docs.conda.io/en/latest/miniconda.html) (64-bit, use default options). Then open a terminal window (or Anaconda command prompt if on Windows), change directories to the `ppd430` folder on your desktop, and run the following commands, one at a time:
...
When you come into class each day, before the lecture begins, do the following steps (takes <1 minute):
  ...
  1. Open a terminal, change directories to the `ppd430` folder on your desktop that you created in step 1 of the initial software setup
  1. Run `git pull` to bring your local clone of the course repo up to date with the remote
  1. Run `conda activate ppd430 && jupyter lab` to start your Jupyter server
